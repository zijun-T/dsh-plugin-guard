...
import {
  createTracker, heavyLoad, hostHeavyInFlight, noteAttempt, noteEvent, noteHeavySettled,
  noteHumanMessage, noteResult, noteTurnEnd, noteTurnStart, noteUsage, tokenTotals,
} from './store.js';
import { evaluate, summarize } from './effort.js';
import { decide, heavyDenyReason, hostHeavyDenyReason, nudgeMessage, shedHeavyReason } from './decisions.js';
import { createLiveness } from './liveness.js';
import { createAudit } from './audit.js';
import { createRecoveryState, recordAction, recoveryDecision } from './recovery.js';
import {
...
export function apply(ctx, rawConfig = {}) {
  ...
  function tick() {
    const now = Date.now();
    safe(() => checkRecovery(now), onError)();
  ...
  /** 插件自己拉起的守卫子进程：对付"事件循环再也不回来"（进程内无解的那种）。 */
  function spawnHelper() {
    if (!cfg.recovery.enabled || !cfg.recovery.helper) return;
    try {
      const helperPath = fileURLToPath(new URL('./helper.mjs', import.meta.url));
      if (!existsSync(helperPath)) return;
      const port = ctx.get(WEB_SERVER_KEYS[0])?.port ?? ctx.get(WEB_SERVER_KEYS[1])?.port ?? 3080;
      const url = cfg.recovery.healthUrl || `http://127.0.0.1:${port}/plugins/dsh-guard/health`;
      helperChild = spawn(process.execPath, [
        helperPath, `--url=${url}`, `--wrapper-pid=${process.ppid}`,
        `--wrapper-pattern=${cfg.recovery.wrapperPattern}`,
        `--interval=${Math.round(cfg.recovery.helperIntervalMs / 1000)}`,
        `--timeout=${Math.round(cfg.recovery.helperTimeoutMs / 1000)}`,
        `--fails=${cfg.recovery.helperFails}`,
        `--cooldown=${Math.round(cfg.recovery.cooldownMs / 1000)}`,
        `--max-per-hour=${cfg.recovery.maxPerHour}`,
        `--state-dir=${join(guardRoot(), 'helper')}`,
      ], { detached: true, stdio: 'ignore' });
      helperChild.unref();
      log('info', `dsh-guard: 守卫子进程已拉起（pid=${helperChild.pid}，探 ${url}；wrapper=${process.ppid}）`);
      void audit?.write({ kind: 'recovery/helper-spawned', pid: helperChild.pid, url, wrapper: process.ppid });
    } catch (error) {
      onError(error);
    }
  }
  ...
  /**
   * 自恢复判定：循环被卡住时插件自己的定时器会"迟到"，所以在每次被释放的瞬间检查
   * "我迟到了多久"。超阈值 ⇒ 主动退出交给 systemd 重拉（不需要 root）。
   */
  function checkRecovery(now) {
    const rc = cfg.recovery;
    if (!rc.enabled) return;
    const stats = liveness.stats();
    const lagMs = Math.max(stats.lastMs, 0);
    const managed = Boolean(process.env.INVOCATION_ID || process.env.JOURNAL_STREAM);
    const disabled = existsSync(join(guardRoot(), 'recovery', 'DISABLED')) || existsSync(join(guardRoot(), 'watchdog', 'DISABLED'));
    const decision = recoveryDecision({ cfg, mode: cfg.mode, now, lagMs, state: recoveryState, managed, disabled });
    if (decision.reason === 'below-threshold') { recoveryState.streak = 0; return; }
    if (decision.reason === 'confirming') { recoveryState.streak = decision.streak ?? 0; return; }
    if (decision.action === 'none') return;
    if (decision.action === 'alert') {
      if (now - (recoveryState.lastAlertAt ?? 0) < rc.cooldownMs) return;
      recoveryState.lastAlertAt = now;
      void audit?.write({ kind: 'recovery/alert', reason: decision.reason, lagMs, managed, streak: decision.streak });
      log('warn', `dsh-guard: 检测到卡死（滞后 ${lagMs}ms）但无法自动恢复：${decision.reason}`);
      return;
    }
    const next = recordAction(recoveryState, now);
    recoveryState.lastActionAt = next.lastActionAt;
    recoveryState.actions = next.actions;
    recoveryState.streak = 0;
    void audit?.write({ kind: 'recovery/self-exit', lagMs, threshold: rc.lagThresholdMs, managed, pid: process.pid });
    log('warn', `dsh-guard: 事件循环滞后 ${lagMs}ms 超过阈值 ${rc.lagThresholdMs}ms ⇒ 主动退出，交由 systemd 重拉`);
  ...
  ctx.effect(() => {
    ...
    return () => {
      ...
      try { helperChild?.kill('SIGTERM'); } catch { /* 随宿主一起退场即可 */ }
  ...
  ctx.effect(() => ctx.tools.guard((exec) => {
    try {
      ...
      if (isHeavyCall(String(exec.name), exec.arguments ?? {}, cfg)) {
        ...
        const lagMs = cfg.liveness.shedUseP95 ? Math.max(livenessStats.lastMs, livenessStats.p95Ms) : livenessStats.lastMs;
        // ① 泄压：循环已经在滞后 ⇒ 先停止加压
        const shed = shedHeavyReason(lagMs, cfg);
        if (shed) {
          void audit?.write({ kind: 'action/shed-heavy', session: id, tool: exec.name, lagMs, liveness: livenessStats });
          return shed;
        }
        // ② 单会话闸：别自己叠
        ...
        // ③ 宿主级闸：别多个人一起开工（卡死的直接成因）
        const host = hostHeavyInFlight(trackers.values(), cfg, now, exec.callId);
        const hostReason = hostHeavyDenyReason(host, cfg);
        if (hostReason) {
          void audit?.write({ kind: 'action/denied-heavy-host', session: id, tool: exec.name, hostInFlight: host });
          return hostReason;
        }
      }
