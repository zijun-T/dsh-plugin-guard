...
export const DEFAULTS = {
  ...
  heavy: {
    ...
    maxConcurrent: 1,
    /**
     * Concurrent heavy calls across the WHOLE host (all sessions/subagents).
     * 卡死的直接成因是"多个成员同时跑克隆/全量套件"把单事件循环压住；实测一次
     * 恢复里 6 个成员同时被唤醒。默认 2：允许并行，但杜绝"六个一起开工"。
     */
    hostMaxConcurrent: 2,
    /**
  ...
  liveness: {
    ...
    lagCriticalMs: 2_000,
    /**
     * 泄压闸：事件循环滞后超过这个值时，enforce 模式会**临时拒绝新的重量级调用**，
     * 直到滞后回落。这是"卡死前的刹车"——在循环还没被彻底占死之前先停止加压。
     */
    shedHeavyAboveMs: 2_000,
    /** 泄压判定用的采样口径：last=当前一次，p95=近期分位（默认用 p95 更稳）。 */
    shedUseP95: true,
  },
  ...
  /**
   * 自恢复（把"看门狗"做进插件本体，不需要 sudo、不需要 systemd 单元）。
   * 两种机制见 lib/recovery.js：插件内自杀式重启 + 插件自己拉起的守卫子进程。
   */
  recovery: {
    enabled: true,
    /** 允许 `process.exit(1)` 交给 systemd 重拉（仅当确属 systemd 托管时才会执行）。 */
    selfExit: true,
    /** 单次事件循环滞后达到这个值即视为"卡死"（实测最坏 45s）。 */
    lagThresholdMs: 30_000,
    /** 需要连续 N 次超阈值观测才动手（防抖）。 */
    confirmations: 1,
    cooldownMs: 600_000,
    maxPerHour: 3,
    /** 插件装载时 spawn 守卫子进程（对付"循环再也不回来"）。 */
    helper: true,
    helperIntervalMs: 5_000,
    helperTimeoutMs: 3_000,
    helperFails: 3,
    /** 空 = 用当前 web 端口自动拼 /plugins/dsh-guard/health。 */
    healthUrl: '',
    /**
     * 守卫子进程唯一允许击杀的目标特征：命令行里必须含它。
     * 绝不信任 ppid —— ppid 可能是无关进程（实测：测试环境里就是 shell）。
     */
    wrapperPattern: 'dsh-web-wrapper.mjs',
  },
...
export function resolveConfig(patch = {}) {
  ...
  const numeric = [
    ...
    ['heavy.hostMaxConcurrent', cfg.heavy.hostMaxConcurrent],
    ...
    ['liveness.shedHeavyAboveMs', cfg.liveness.shedHeavyAboveMs],
    ['recovery.lagThresholdMs', cfg.recovery.lagThresholdMs], ['recovery.confirmations', cfg.recovery.confirmations],
    ['recovery.cooldownMs', cfg.recovery.cooldownMs], ['recovery.maxPerHour', cfg.recovery.maxPerHour],
    ['recovery.helperIntervalMs', cfg.recovery.helperIntervalMs], ['recovery.helperTimeoutMs', cfg.recovery.helperTimeoutMs],
    ['recovery.helperFails', cfg.recovery.helperFails],
  ];
